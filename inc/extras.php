...
function rootstrap_post_before() {
    rootstrap_do_contextual_hook('rootstrap_post_before');
}

function rootstrap_post_after() {
    rootstrap_do_contextual_hook('rootstrap_post_after');
}

function rootstrap_post_start() {
    rootstrap_do_contextual_hook('rootstrap_post_start');
}

function rootstrap_post_end() {
    rootstrap_do_contextual_hook('rootstrap_post_end');
}

/* page.php ------------------------------------------------------------------- */

function rootstrap_page_before() {
    rootstrap_do_contextual_hook('rootstrap_page_before');
}

function rootstrap_page_after() {
    rootstrap_do_contextual_hook('rootstrap_page_after');
}

function rootstrap_page_start() {
    rootstrap_do_contextual_hook('rootstrap_page_start');
}

function rootstrap_page_end() {
    rootstrap_do_contextual_hook('rootstrap_page_end');
}

/* single.php, page.php, templates with comments ------------------------------ */

function rootstrap_comments_before() {
    rootstrap_do_contextual_hook('rootstrap_comments_before');
}

function rootstrap_comments_after() {
    rootstrap_do_contextual_hook('rootstrap_comments_after');
}

/* sidebar.php ---------------------------------------------------------------- */

function rootstrap_sidebar_before() {
    rootstrap_do_contextual_hook('rootstrap_sidebar_before');
}

function rootstrap_sidebar_after() {
    rootstrap_do_contextual_hook('rootstrap_sidebar_after');
}

function rootstrap_sidebar_start() {
    rootstrap_do_contextual_hook('rootstrap_sidebar_start');
}

function rootstrap_sidebar_end() {
    rootstrap_do_contextual_hook('rootstrap_sidebar_end');
}

/* footer.php ----------------------------------------------------------------- */

function rootstrap_content_end() {
    rootstrap_do_contextual_hook('rootstrap_content_end');
}

function rootstrap_footer_before() {
    rootstrap_do_contextual_hook('rootstrap_footer_before');
}

function rootstrap_footer_after() {
    rootstrap_do_contextual_hook('rootstrap_footer_after');
}

function rootstrap_footer_start() {
    rootstrap_do_contextual_hook('rootstrap_footer_start');
}

function rootstrap_footer_end() {
    rootstrap_do_contextual_hook('rootstrap_footer_end');
}

function rootstrap_body_end() {
    rootstrap_do_contextual_hook('rootstrap_body_end');
}
...
function rootstrap_social() {
    $services = array('facebook', 'twitter', 'googleplus', 'youtube', 'linkedin', 'pinterest', 'rss', 'tumblr', 'flickr', 'instagram', 'dribbble', 'yelp', 'map-marker', 'yahoo', 'search', 'circle-o', 'slideshare', 'wordpress', 'medium');
    ...
    foreach ($services as $service) :
        ...
        if ($active[$service]) {
            echo '<li><a class="social-profile" href="' . $active[$service] . '" class="social-icon ' . $service . '" title="' . __('Follow us on ', 'rootstrap') . $service . '"><i class="social_icon fa fa-' . $service . '"></i></a></li>';
        }
...
// //Display social links only
function rootstrap_social_links() {
    $services = array('facebook', 'twitter', 'googleplus', 'youtube', 'linkedin', 'pinterest', 'rss', 'tumblr', 'flickr', 'instagram', 'dribbble', 'yelp', 'map-marker', 'yahoo', 'search', 'circle-o', 'slideshare', 'wordpress', 'medium');
    $hide_mobile = rootstrap_get_option('social_hide-mobile');
    ...
    foreach ($services as $service) :
        ...
        $active[$service] = rootstrap_get_option('social_' . $service);
        if ($active[$service]) {
            echo '<li ' . (($hide_mobile[$service]) ? 'class="hidden-xs"' : '') . '><a class="' . $service . '" href="' . $active[$service] . '" title="' . __('Follow us on ', 'rootstrap') . $service . '"  target="_blank"><i class="social_icon fa fa-' . $service . '"></i></a></li>';
        }
...
// //Display site phone
function rootstrap_display_site_info($arg) {
    $data = rootstrap_get_option('site_' . $arg);

    if ($data) {
        echo '<p class="contact-' . $arg . '">';
        if ($arg == 'phone') {
            echo '<i class="fa fa-' . $arg . '"></i>';
            echo '<a href="callto:' . $data . '">';
        } else {
            echo '<i class="fa fa-envelope"></i>';
            echo '<a href="mailto:' . $data . '">';
        }
        echo $data . '</a></p>';
    }
}
...
function rootstrap_theme_topmenu() {
    ?>    
    <div class="top-link">
    ...
    <?php rootstrap_display_site_info('phone'); ?>
    <?php rootstrap_social_links(); ?>                        
                    </div>
                    <div class="col-md-3 col-sm-12">
                        <div class="right-align">
    <?php rootstrap_display_site_info('mail'); ?>
                        </div>
                    </div>
                </div>
            </div>
...
add_action('before', 'rootstrap_theme_topmenu');
remove_action('before', 'rootstrap_theme_topmenu_social');
...
function rootstrap_featured_slider() {
    if (is_front_page() && rootstrap_get_option('rootstrap_slider_checkbox') == 1) {
        ...
        $query = new WP_Query(array('post_type' => $slidecat, 'posts_per_page' => $count));
//          $query = new WP_Query( array( 'cat' =>$slidecat,'posts_per_page' =>$count ) );
        if ($query->have_posts()) :
            while ($query->have_posts()) : $query->the_post();
                ...
                echo the_excerpt();
                echo '<a class="da-link" href="' . get_permalink() . '">Read More</a>';
...
if (!function_exists('get_rootstrap_theme_options')) {
    ...
    function get_rootstrap_theme_options() {
        ...
        /* Scripts Before wp_head */
        if (rootstrap_get_option('custom_header')) {
            echo rootstrap_get_option('custom_header');
        }
...
// Add scripts at footer
function get_rootstrap_footer_scripts() {
    /* Scripts Before wp_footer */
    if (rootstrap_get_option('custom_footer')) {
        echo rootstrap_get_option('custom_footer');
    }
}
add_action('wp_footer', 'get_rootstrap_footer_scripts', 10);
...
/* Add Silder & Testimonial post-type: where title=>client name, editor=>content, thumbnail=>photo */

function rootstrap_register_post_types() {
    $types = array('slider', 'testimonial');
    foreach ($types as $type) {
        $camel_type = ucwords(strtolower($type));
        $labels = array(
            'name' => $camel_type . 's',
            'singular_name' => $camel_type,
            'add_new' => 'Add New',
            'add_new_item' => 'Add New ' . $camel_type,
            'edit_item' => 'Edit ' . $camel_type,
            'new_item' => 'New ' . $camel_type,
            'view_item' => 'View ' . $camel_type,
            'search_items' => 'Search ' . $camel_type . 's',
            'not_found' => 'No ' . $camel_type . 's found',
            'not_found_in_trash' => 'No ' . $camel_type . 's in the trash',
            'parent_item_colon' => '',
        );
...
add_action('init', 'rootstrap_register_post_types');
